					...
					{#if colorScheme === video.scheme}
						{#if data.ref === 'customizable'}
							<FadeIn inview={observer} delay={.3} inheritbg={true}>
								...
								on:timeupdate={scaleOnTime} bind:currentTime={custCurrentTime} bind:duration={custDuration} style="transition: transform 0.1s; transform: scale(2.4); transform-origin: bottom;">
									<source src={video.src} type={video.type} />
									{browserSupportText}
								</video>
							</FadeIn>
